...
def extract_frames_from_video(
    video_bytes: bytes, interval: int = 10
) -> List[np.ndarray]:
...
# 데이터 증강
def augment_image(image: np.ndarray, use_flip=True) -> List[np.ndarray]:
    aug_images = [image]
...
# 증강 적용 함수
def occlusion_augment(image: np.ndarray, region_list=None):
    if region_list is None:
        region_list = ["mask", "sunglasses"]
